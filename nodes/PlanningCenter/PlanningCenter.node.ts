    ...
    for (let i = 0; i < items.length; i++) {
      try {
        ...
        if (operation === 'getManyPeople' || operation === 'getPerson') {
          const whereFilters = this.getNodeParameter(
            'peopleFilters.whereFilters',
            i,
            [],
          ) as Array<{
            field: string;
        ...
        if (resource === 'people') {
          if (operation === 'getPerson') {
            ...
            const fieldDefinitionId = this.getNodeParameter(
              'fieldDefinitionId',
              i,
              '',
            ) as string;
          ...
          if (operation === 'getChannel') {
            ...
            if (servicesPlanRemoteIdentifier)
              qs['where[services_plan_remote_identifier]'] =
                servicesPlanRemoteIdentifier;
