...
- Build a metadata registry for all People operations and generate both UI and execution from that registry.
...
- Every People endpoint must exist in one registry entry.
...
- Build a normalized inventory for all People resources and operations from the bundled docs.
...
- Add a coverage report that proves registry coverage against the bundled docs.
...
- Coverage check proves `docs inventory == registry inventory`.
...
- Add a registry coverage test that compares the internal operation manifest to the bundled docs.
- Add unit tests for:
  - path interpolation
  - query serialization
  - pagination
  ...
  - response normalization
- Add representative snapshot tests for generated n8n properties.
...
- All 502 documented People operations exist in the registry.
